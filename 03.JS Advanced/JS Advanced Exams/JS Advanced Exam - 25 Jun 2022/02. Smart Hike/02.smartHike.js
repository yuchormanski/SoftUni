class SmartHike {
    ...
    showRecord(criteria) {
        ...
        if (criteria === 'all') {
            ...
            const found = this.listOfHikes.filter(crit => crit.difficultyLevel === criteria);
            if (found.length === 0) {
                return `${this.username} has not done any ${criteria} hiking yet`
            }
            found.sort((a, b) => a.time - b.time);
            return `${this.username}'s best ${criteria} hike is ${found[0].peak} peak, for ${found[0].time} hours`
        }
